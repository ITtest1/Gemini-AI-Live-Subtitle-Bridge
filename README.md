...
## 🛠️ Manual Self-Hosting Guide / 手動搭建伺服器指南

If you prefer to run the server from source, follow these steps: / 若您偏好從原始碼運行伺服器，請遵循以下步驟：

## 🐳 Docker Deployment / Docker 部署指南

For a streamlined deployment, you can use Docker Compose: / 若希望快速部署，您可以使用 Docker Compose：

### 1. Requirements / 需求
- [Docker](https://docs.docker.com/get-docker/)
- [Docker Compose](https://docs.docker.com/compose/install/)
```
git clone https://github.com/ITtest1/Gemini-AI-Live-Subtitle-Bridge.git
cd Gemini-AI-Live-Subtitle-Bridge
```
### 2. Deployment / 部署步驟
1. **Configure Environment:** / **設定環境:**
   Create a `.env` file in the root directory (if not exists) and add your `GEMINI_API_KEY`: / 在根目錄建立 `.env` 檔案並填入您的 `GEMINI_API_KEY`:
   ```env
   GEMINI_API_KEY=your_key_here
   ```
2. **Start the service:** / **啟動服務:**
   ```bash
   docker-compose up -d
   ```
   The service will be available at `http://localhost:8081` (or the port defined in `docker-compose.yml`). / 服務將在 `http://localhost:8081` 運行。

---

### 1. Prerequisites / 環境需求
- [Node.js](https://nodejs.org/) (Version 18 or above / 版本 18 或以上)
- npm / yarn
...
1. **Install dependencies:** / **安裝依賴:**
   ...
   npm install
   ```
2. **Configure environment:** / **設定環境:**
   Create a `.env` file in the root directory and add your `GEMINI_API_KEY`: / 在根目錄建立 `.env` 檔案並填入您的 `GEMINI_API_KEY`:
   ```env
   GEMINI_API_KEY=your_key_here
   ```
3. **Start the server:** / **啟動伺服器:**
   - Development mode: / 開發模式: `npm run dev`
   - Production mode: / 生產模式: `npm run build && npm start`
...
![總覽截圖](pic/20260624_225242.jpg)

<details>
<summary>點擊查看更多細節截圖</summary>

![截圖1](pic/celluloid-shot0001.jpg)
![截圖2](pic/celluloid-shot0002.jpg)
![截圖3](pic/celluloid-shot0003.jpg)
![截圖4](pic/celluloid-shot0004.jpg)
![截圖5](pic/celluloid-shot0005.jpg)
![截圖6](pic/celluloid-shot0009.jpg)
![截圖7](pic/celluloid-shot0010.jpg)
![截圖8](pic/celluloid-shot0011.jpg)
![截圖9](pic/celluloid-shot0012.jpg)
![截圖10](pic/celluloid-shot0017.jpg)
![截圖11](pic/celluloid-shot0019.jpg)
